...
class GamJam extends Program {
    
    TransparentImage window = newTransparentImage(HAUTEUR, LARGEUR);
    TransparentImage background = newTransparentImage("./ressources/background.png");
    ...
    final int LONGUEURMONDE=MIN+(int)(Math.random()*(MAX-MIN)+1);
    final int LARGUEURMONDE=MIN+(int)(Math.random()*(MAX-MIN)+1);
	...
	drawImage(img, copyAndResize(poulet, "poulet-petit",70,50) , HAUTEUR/4, LARGEUR/3);
	return newImg;
    }
	...
	//creation de mon monde
	final int LONGUEURMONDE=MIN+(int)(Math.random()*(MAX-MIN)+1);
	final int LARGUEURMONDE=MINLARG+(int)(Math.random()*(MAXLARG-MINLARG)+1);
	monde=new int [LONGUEURMONDE][LARGUEURMONDE];
       
	//placement de mon pion
	x=10;
	y=2;
	...
	for(int i=0;i<monde.length;i++){
		...
		int random=probabilitéApparitionEnnemi();
    ...
    int probabilitéApparitionEnnemi(){
	...
	//transforme le fond en noir
	background("black");
	
	//affichage des caractéristiques de notre personnage
	print("          Nombre de point = "+nbPoint+"\n          Point de vie = "+pointDeVie+"\n          Niveau Joueur="+niveau);
	cursor(100,100);
    }
